...
class Ship
  ...
  step: (o) ->
    [@o, @thrust, @torque, @label] = [o, 0, 0, 'ಠ_ಠ']
    @finishStep()
...
class Drone extends Ship
  constructor: ->
    @key = ""
    @key += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(Math.random() * "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".length)) for i in [0..5]
  ...
  reportIn: ->
    if @o.mothership.fleet.indexOf(@key) is -1 then @o.mothership.fleet.push @key

class Queen extends Ship
  step: (o) ->
    super o
...
ai.step = (o) ->
  if ship instanceof Ship
    ...
    ship = if o.me.queen then new Queen() else new Drone()
  ship.step o
